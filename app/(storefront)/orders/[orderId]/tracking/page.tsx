...
export default async function OrderTrackingPage({ params }: OrderTrackingPageProps) {
  ...
  const history = [
    ...
    ...(order.status !== OrderStatus.PENDING
      ? [{ label: `Status updated to ${STEP_LABELS[order.status]}`, timestamp: order.updatedAt }]
      : []),
  ...
  return (
    <main className="mx-auto max-w-180 px-6 py-12">
      ...
      <p className="mb-8 font-mono text-[13px] text-gray-500">
        {orderNumber} · Placed {format(order.createdAt, "MMM d, yyyy")}
      </p>
              ...
              return (
                ...
                >
                  {index > 0 && (
                    <div
                      className={cn(
                        "absolute top-2.5 left-[-50%] h-0.5 w-full",
      ...
      <div className="flex flex-col gap-3.5">
        {history.map((entry) => (
          <div key={`${entry.label}-${entry.timestamp.toISOString()}`} className="flex items-start gap-3.5">
