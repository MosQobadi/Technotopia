...
export default function StorefrontDevPreviewPage() {
  return (
    // This page lives outside the storefront's [locale] tree (dev-only
    // tooling, see robots.ts), so the storefront primitives it previews —
    // which now call useTranslations/next-intl's Link — need their own
    // provider here instead of inheriting one from a parent layout.
    <NextIntlClientProvider locale="en" messages={enMessages}>
      <StorefrontDevPreviewContent />
    </NextIntlClientProvider>
  );
}

function StorefrontDevPreviewContent() {
    ...
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col gap-10 p-8">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Temporary page to confirm the storefront design tokens and shared UI primitives (Task
          15.5) — remove once confirmed.
        </p>
      </div>
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <OrderStatusBadge status={OrderStatus.SENDING} />
        </div>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <Breadcrumb
          items={[
            { label: "Home", href: "/" },
            { label: "Microphones", href: "/products" },
            { label: "Cardio X2 Condenser Mic" },
          ]}
        />
      </section>
