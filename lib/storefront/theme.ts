...
// of the selectors @heroui/styles keys its own dark variables on (see
// themes/default/variables.css), and app/globals.css points Tailwind's `dark:`
// variant at the same attribute, so setting it repaints our semantic tokens and
// HeroUI's components in one step. Nothing else needs to know the theme —
...
// that is not cosmetic. React owns `<html className>` — app/[locale]/layout.tsx
// renders `h-full antialiased` there — and it rewrites that attribute on any
// client-side navigation that re-renders the layout. Switching locale does
// exactly that, and a class added by the script below is not in React's vdom,
// so it would be wiped on the way from /en to /fa: dark theme in, light theme
// out, with localStorage still saying dark. React never touches `data-theme`,
// so it survives.
...
export const THEME_STORAGE_KEY = "technotopia-theme";

/**
 * Runs before first paint, inlined as the first child of <body> — see
 * app/[locale]/layout.tsx. Without it the page renders light, then flips once
 * React hydrates, which is a white flash on every cold load for anyone using
 * the dark theme.
 *
 ...
 * different `[locale]` root layout, and React resets <html> to exactly the props
 * it renders: lang, dir and class, and nothing else. Anything a script put there
 * is gone, and the inline script that would have restored it does not re-run on
 * a client-side navigation.
 *
 * The obvious alternative is to let React own the value so it survives by being
 * re-rendered, which means the server must know it, which means a cookie — and a
 * `cookies()` read in the layout would opt the storefront out of the static
 * rendering `setRequestLocale` and the routes' `revalidate = 300` exist to get.
 * Not worth it for one attribute.
 */
...
/**
 * Applies a choice and remembers it. Storing on every toggle rather than only on
 * the first is what keeps a second tab, or the next visit, in step; a write that
 * throws still leaves the current page correctly painted, which is the half that
 * matters to the person who just clicked.
 */
